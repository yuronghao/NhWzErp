...
<script type="text/javascript">
		...
		function getsoblist(){
			window.location.href = '${ctx}/wms/purchase_toAddpurchaseList.emi';
		}
		 ...
		 <div class="EMonecontent">
		 	...
		 	<div class="toolbar">
		 		<ul>
		 			...
		 			<%--<li class="fl"><input type="button" class="btns" value="新增" id="addBtn"  disabled> </li>--%>
		 		...
		 		<div>
		 			...
		 			<ul class="wordul">
		 				...
		 				<li class="wordli fl">
							...
							<div class="wordnameinput fl">
								<input type="text" value="${purchase['cPTName']}" id="cPTName" name="cPTName" readonly="readonly">
								<%--<select id="procureType" name="procureType" disabled="disabled">--%>
									<%--<c:forEach items="${purchaseTypelist}" varStatus="status" var="purchasetype">--%>
										<%--<option value="${purchasetype.pk}"<c:if test="${purchase['procureType'] == purchasetype.pk}">selected="selected"</c:if>>${purchasetype.cPTName}</option>--%>

									<%--</c:forEach>--%>

									<%--&lt;%&ndash;<option value="1"<c:if test="${purchase['procureType']==1}">selected="selected"</c:if>>固定资产采购</option>&ndash;%&gt;--%>
								<%--</select>--%>
							</div>
							<div class="cf"></div> 
		 				</li>
		 			...
		 			<li class="wordli fl">
		 				...
		 				<li class="wordli fl">
							...
							<div class="wordnameinput fl"><input type="text" id="rate" name="rate" class="toDealInput" value="<fmt:formatNumber type="number" value="${purchase['rate']}" minFractionDigits="2" groupingUsed="false"/>"> </div>
							<div class="cf"></div> 
		 				</li>
		 				<div class="cf"></div> 
		 			</ul>
		 			<ul class="wordul">
		 				<li class="wordli fl">
							...
							<div class="wordnameinput fl"><input type="text" id="exchangeRate" name="exchangeRate" class="toDealInput" value="<fmt:formatNumber type="number" value="${purchase['exchangeRate']}" minFractionDigits="2" groupingUsed="false"/>"> </div>
							<div class="cf"></div> 
		 				</li>
		 				 <%--<li class="wordli fl">--%>
		 			...
		 			<div style="max-height:300px;overflow:auto;">
			 			<table>
				 			<tbody id="contr">
								...
								<input type="hidden" id="goodsArray"  name="goodsArray" value="">
				 			<c:forEach var="type" items="${purchasec}" varStatus="stat">
				 		    <tr class="serialTr">
								<td style="width: 50px;"><input type="checkbox" id="strsum" class="strsum" name="strsum" value="${type.goodsuid}" goodsCode="${type.good.goodscode}" goodsName="${type.good.goodsname}" goodsstandard="${type.good.goodsstandard}"/> </td>
			                ...
			                <td class="number"><input type="text" id="" name="number" class="listword toDealInput" value="<fmt:formatNumber type="number" value="${type.number}" minFractionDigits="2" groupingUsed="false"/>"></td>
			                <td class="unitName"><input type="text" id="" name="unitName" class="listword" value="${type.unit.unitname}" readonly="readonly"></td>
			                ...
			                <td class="originalTaxPrice"><input type="text" id="" name="originalTaxPrice" class="listword toDealInput" value="<fmt:formatNumber type="number" value="${type.originalTaxPrice}" minFractionDigits="4" groupingUsed="false"/>"></td>
			                <td class="originalTaxMoney"><input type="text" id="" name="originalTaxMoney" class="listword toDealInput" value="<fmt:formatNumber type="number" value="${type.originalTaxMoney}" minFractionDigits="2" groupingUsed="false"/>"></td>
			                <td class="originalNotaxPrice"><input type="text" id="" name="originalNotaxPrice" class="listword toDealInput" value="<fmt:formatNumber type="number" value="${type.originalNotaxPrice}" minFractionDigits="4" groupingUsed="false"/>"></td>
			                <td class="originalNotaxMoney"><input type="text" id="" name="originalNotaxMoney" class="listword toDealInput" value="<fmt:formatNumber type="number" value="${type.originalNotaxMoney}" minFractionDigits="2" groupingUsed="false"/>"></td>
			                <td class="originalTax"><input type="text" id="" name="originalTax" class="listword toDealInput" value="<fmt:formatNumber type="number" value="${type.originalTax}" minFractionDigits="2" groupingUsed="false"/>"></td>
			                ...
			                <td class="putinNumber"><input type="text" id="" name="putinNumber" class="listword" value="<fmt:formatNumber type="number" value="${type.putinNumber}" minFractionDigits="2" groupingUsed="false"/>" readonly="readonly"></td>
...
<script>
	//定义使用对象
    function toolprenatalexam(goodsUid,goodsCode,goodsName,goodsstandard,printamount){
    ...
    function openprintpage(){
        var a = $("input[name='strsum']:checked");
        if(a.length <=0){
            alert("请先选择打印物品");
            return false;
		}
        ...
        $("#goodsUids").val("");
        var newGodosArray = new Array();//对象数组
        var goodsarray = $(".strsum");
        $(goodsarray).each(function(i,j){
            if(this.checked){
                var goodsuid  = $(this).val();

                var goodsCode  = $(this).attr("goodsCode");
                var goodsName  = $(this).attr("goodsName");
                var goodsstandard  = $(this).attr("goodsstandard");
                var putinNumber = $(this).parent().parent().find("input[name='printamount']").val();
                var toolprenatalexamaaa = new toolprenatalexam(goodsuid,goodsCode,goodsName,goodsstandard,putinNumber);
                newGodosArray.push(toolprenatalexamaaa);
			}
		})
		$("#goodsArray").val(JSON.stringify(newGodosArray));
        ...
        $.dialog({
            ...
            width: '300px',
            height: '150px',
            content: 'url:wms/purchase_toopenprintpage.emi',
            okVal:"打印",
            ok:function(){
				...
				$.ajax({
					url:'${ctx}/wms/purchase_printbarcode.emi?printservice='+encodeURI(printservice)+"&printmodel="+encodeURI(printmodel),
					type:"post",
					data:$('#myform').serialize(),
					success:function(da){
						if(da=='success'){
