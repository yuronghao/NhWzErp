...
<body>
<form action="${ctx}/wms/produceprocess_dispatchingStartList.emi" name="myform" id="myform" method="post">
...
<!-- 			 					<th>设备编码</th>
							...
							<tr>
								...
								<td title="${bean.billCode}" docgid="${bean.disGid }" class="billcode">${bean.billCode}</td>
								...
								<td title="${bean.startTime}">${fn:substring(bean.startTime,0,19)}</td>
								<td title="${bean.dispatchingObjName}">${bean.dispatchingObjName}</td>
								<td title="${bean.disNum}"><fmt:formatNumber type="number" groupingUsed="false" maxFractionDigits="2" value="${bean.disNum}"></fmt:formatNumber></td>
		...
		<script type="text/javascript">
			$(function(){
				$('.billcode').dblclick(function(){
					...
					var gid=$(this).attr("docgid");
					window.parent.CreateDiv('erp_03_03_06',"${ctx}/wms/produceprocess_toStartWork.emi?disOrderGid="+gid,'开工',true);
				});
			});
		</script>
