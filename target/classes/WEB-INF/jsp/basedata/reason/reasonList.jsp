...
<head>
	...
	//新增
	function addRoute(){
		location.href = "${ctx}/wms/reason_toAddreason.emi";
	}
	//编辑基本信息
	function editRoute(){
		var reasonId = checkSelectId('userCheck');
		if(reasonId!=''){
			location.href = "${ctx}/wms/reason_toUpdatereason.emi?reasonId="+reasonId;
		}
	}
		...
		if (gid!=''){
			$.dialog.confirm(tip_confirmDelete,function(){
				$.ajax({
					...
					type: 'POST',
					url: "${ctx}/wms/reason_deletereason.emi?reasonId="+gid,
...
<body>
	<form action="${ctx}/wms/reason_getreasonList.emi" name="myform" id="myform" method="post" >
			...
			<table>
				<tbody>
					<tr>
						...
						<th width="20%" style="text-align: center;">原因编码</th>
						<th width="20%" style="text-align: center;">原因名称</th>
					</tr>
					<c:forEach var="reason" items="${data.list }" varStatus="stat">
						<tr>
							<td><input type="checkbox" name="userCheck" value="${reason.gid }" onclick="clickCheck();"></td>
