...
<head>
	...
	//新增
	function addRoute(){
		location.href = "${ctx}/wms/costItem_toAddcostItemSourceSet.emi";
	}
	//编辑基本信息
	function editRoute(){
		var costItemId = checkSelectId('userCheck');
		if(costItemId!=''){
			location.href = "${ctx}/wms/costItem_toUpdatecostItemSourceSet.emi?costItemId="+costItemId;
		}
	}
		...
		if (gid!=''){
			$.dialog.confirm(tip_confirmDelete,function(){
				$.ajax({
					...
					type: 'POST',
					url: "${ctx}/wms/costItem_deletecostItemSourceSet.emi?costItemSourceSet="+gid,
...
<body>
	<form action="${ctx}/wms/costItem_costItemSourceSetList.emi" name="myform" id="myform" method="post" >
	...
	<div class="EMonecontent" style="padding-top: 10px">
		...
		<div class="creattable">
	 		<div class="tabletitle">成本项目取数列表</div>		 		
			...
			<table>
				<tbody>
					<tr>
						...
						<th width="15%" style="text-align: center;">来源设置</th>
						<th width="20%" style="text-align: center;">出入库类别</th>
						<th width="10%" style="text-align: center;">部门</th>
						<th width="15%" style="text-align: center;">科目编码</th>
						<th width="15%" style="text-align: center;">备注</th>
					</tr>
					<c:forEach var="costItemSourceSetList" items="${data.list }" varStatus="stat">
						<tr>
							<td><input type="checkbox" name="userCheck" value="${costItemSourceSetList.gid }" onclick="clickCheck();"></td>
							<td>${stat.count+(data.pageIndex-1)*(data.pageSize) }</td>
							<td>${costItemSourceSetList.costItemName }</td>
							<td>${costItemSourceSetList.apName }</td>
							<td>${costItemSourceSetList.rdStyleName }</td>
							<td>${costItemSourceSetList.depName }</td>
							<td>${costItemSourceSetList.subjectCode }</td>
							<td>${costItemSourceSetList.notes }</td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
