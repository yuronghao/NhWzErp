...
<head>
	...
	//新增
	function addRoute(){
		location.href = "${ctx}/wms/costItem_toAddcostItemAllotRate.emi";
	}
	//编辑基本信息
	function editRoute(){
		var costItemId = checkSelectId('userCheck');
		if(costItemId!=''){
			location.href = "${ctx}/wms/costItem_toUpdatecostItemAllotRate.emi?costItemId="+costItemId;
		}
	}
		...
		if (gid!=''){
			$.dialog.confirm(tip_confirmDelete,function(){
				$.ajax({
					...
					type: 'POST',
					url: "${ctx}/wms/costItem_deletecostItemAllotRate.emi?costItemAllotRateId="+gid,
...
<body>
	<form action="${ctx}/wms/costItem_costItemAllotRateList.emi" name="myform" id="myform" method="post" >
	...
	<div class="EMonecontent" style="padding-top: 10px">
		...
		<div class="creattable">
	 		<div class="tabletitle">成本项目分配率列表</div>		 		
			...
			<table>
				<tbody>
					<tr>
						...
						<th width="15%" style="text-align: center;">成本要素</th>
						<th width="15%" style="text-align: center;">产品编码</th>
						<th width="15%" style="text-align: center;">产品规格</th>
						<th width="15%" style="text-align: center;">产品</th>
						<th width="10%" style="text-align: center;">工序(自由项1)</th>
						<th width="10%" style="text-align: center;">部门</th>
						<th width="10%" style="text-align: center;">系数</th>
						<th width="15%" style="text-align: center;">备注</th>
					</tr>
					<c:forEach var="costItemAllotRateList" items="${data.list }" varStatus="stat">
						<tr>
							<td><input type="checkbox" name="userCheck" value="${costItemAllotRateList.gid }" onclick="clickCheck();"></td>
							<td>${stat.count+(data.pageIndex-1)*(data.pageSize) }</td>
							<td>${costItemAllotRateList.costItemName }</td>
							<td>${costItemAllotRateList.goodsCode }</td>
							<td>${costItemAllotRateList.goodsStandard }</td>
							<td>${costItemAllotRateList.goodsName }</td>
							<td>${costItemAllotRateList.cfree1 }</td>
							<td>${costItemAllotRateList.depName }</td>
							<td>${costItemAllotRateList.ratio }</td>
							<td>${costItemAllotRateList.notes }</td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
	...
	function importRoute(){
		$.dialog({ 
			...
			height: '300px',
			zIndex:2000,
			content: 'url:${ctx}/wms/costItem_toImportCostRatePage.emi',
			okVal:"关闭",
			ok:function(){
				//document.getElementById('comment').value = this.content.document.getElementById('dia_comment').value;
