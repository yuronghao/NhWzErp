...
<head>
	...
	//新增
	function addRoute(){
		location.href = "${ctx}/wms/group_toAddgroup.emi";
	}
	//编辑基本信息
	function editRoute(){
		var groupId = checkSelectId('userCheck');
		if(groupId!=''){
			location.href = "${ctx}/wms/group_toUpdategroup.emi?groupid="+groupId;
		}
	}
		...
		if (gid!=''){
			$.dialog.confirm(tip_confirmDelete,function(){
				$.ajax({
					...
					type: 'POST',
					url: "${ctx}/wms/group_deletegroup.emi?gid="+gid,
...
<body>
	<form action="${ctx}/wms/group_getgroupList.emi" method="post" >
			...
			<table>
				<tbody>
					<tr>
						...
						<th width="20%" style="text-align: center;">组编码</th>
						<th width="20%" style="text-align: center;">条码</th>
						<th width="20%" style="text-align: center;">组名称</th>
						<th width="20%" style="text-align: center;">工作中心名称</th>
					</tr>
					<c:forEach var="group" items="${data.list }" varStatus="stat">
						<tr>
							<td><input type="checkbox" name="userCheck" value="${group.gid }" onclick="clickCheck();"></td>
