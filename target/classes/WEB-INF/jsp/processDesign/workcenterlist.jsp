...
<head>
	...
	//新增
	function addRoute(){
		location.href = "${ctx}/wms/basepd_toAddWorkcenter.emi";
	}
	//编辑基本信息
	function editRoute(){
		var workCenterId = checkSelectId('userCheck');
		if(workCenterId!=''){
			location.href = "${ctx}/wms/basepd_toUpdateWorkcenter.emi?workCenterId="+workCenterId;
		}
	}
		...
		if (gid!=''){
			$.dialog.confirm(tip_confirmDelete,function(){
				$.ajax({
					...
					type: 'POST',
					url: "${ctx}/wms/basepd_deleteWorkCenter.emi?workCenterId="+gid,
...
<body>
	<form action="${ctx}/wms/basepd_toWorkCenterList.emi" method="post" >
	...
	<div class="EMonecontent"  style="padding-top: 10px">
		...
		<div class="creattable">
	 		<div class="tabletitle">工作中心列表</div>		 		
			...
			<table>
				<tbody>
					<tr>
						...
						<th width="20%" style="text-align: center;">工作中心编码</th>
						<th width="20%" style="text-align: center;">工作中心名称</th>
						<th width="20%" style="text-align: center;">部门名称</th>
					</tr>
					<c:forEach var="flow" items="${data.list }" varStatus="stat">
						<tr>
							...
							<td nowrap class="longText" title="${flow.wccode }">${flow.wccode }</td>
							<td nowrap class="longText" title="${flow.wcname }">${flow.wcname }</td>
							<td nowrap class="longText" title="${flow.depName }">${flow.depName }</td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
