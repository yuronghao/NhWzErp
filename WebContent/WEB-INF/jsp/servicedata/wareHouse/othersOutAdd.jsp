...
<script type="text/javascript">
		...
		$(function(){
			...
			$('#saveBtn').click(function(){		
				...
				if(isSave){//新增保存			
					$.ajax({
						url:'${ctx}/wms/wareHouse_addOthersOut.emi',
						type:"post",
						...
						success:function(da){				
							...
							var obj=eval("("+da+")");
							if(obj.success==1){
								window.location.href = '${ctx}/wms/wareHouse_toAddOthersOut.emi';
							}else{
					...
					$.ajax({
						url:'${ctx}/wms/wareHouse_updateOthersOut.emi',
						type:"post",
						data:$('#myform').serialize(),
						success:function(da){					
		...
		function deletesob(gid) {
			if (confirm("是否确定删除?")) {
				$.ajax({
							url : '${ctx}/wms/wareHouse_deleteOthersOut.emi',
							type : 'post',
							data : {gid : gid},
							success : function(da) {	
								var obj=eval("("+da+")");
		...
		//放弃
		function giveup() {
			location.href = '${ctx}/wms/wareHouse_toAddOthersOut.emi';
		}
		
		
		function getprocurearrivallist(){
			window.location.href = '${ctx}/wms/wareHouse_othersOutList.emi';
		}
...
<input type="hidden" id="othersOutgid" name="othersOutgid" value="${otherOut['gid']}">
		 <div class="EMonecontent">
		 	...
		 	<div class="toolbar">
		 		<ul>
		 			...
		 			<li class="fl"><input type="button" class="btns" value="删除" id="delBtn" onclick="deletesob('${otherOut['gid']}')"> </li>
		 			...
		 			<script>
		 				$(function() {
							 ...
							 */
							 var cond = "";
		 					initPageTurning('${ctx }/wms/wareHouse_toAddOthersOut.emi','WM_OthersOut','gid',"${otherOut['gid']}",
		 							cond,'otherOutgid');
		 				});
		 			</script>
		 			<!-- 单据翻页end -->
		 	...
		 	<div class="creattable">
		 		<div class="tabletitle" style="margin-top:6px;">其它出库单</div>
		 		<div>
		 			<!--12-->
		 			<ul class="wordul">
		 				<li class="wordli fl">
							<div class="wordname fl">单据编号：</div>
							<div class="wordnameinput fl"><input type="text" value="${otherOut['billCode']}" id="billCode" name="billCode" readonly="readonly"> </div>
							<div class="cf"></div> 
		 				</li>
		 				<li class="wordli fl">
							<div class="wordname fl">单据日期：</div>
							<div class="wordnameinput fl"><input type="text" value="${fn:substring(otherOut['billDate'],0,10)}" id="billDate" name="billDate" readonly="readonly"> </div>
							<div class="cf"></div> 
		 				</li>
		 				<li class="wordli fl">
		 			...
		 			<%--<ul class="wordul">--%>
		 				<%--&lt;%&ndash;<li class="wordli fl">&ndash;%&gt;--%>
						<%--&lt;%&ndash;<div class="wordname fl">入库标志：</div>&ndash;%&gt;--%>
						<%--&lt;%&ndash;&lt;%&ndash;<div class="wordnameinput fl">&ndash;%&gt;&ndash;%&gt;--%>
							<%--&lt;%&ndash;&lt;%&ndash;<select class="toDealSelect" id="badge" name="badge" onchange="getSelect(this)" disabled="disabled">&ndash;%&gt;&ndash;%&gt;--%>
								<%--&lt;%&ndash;&lt;%&ndash;<option value="1" <c:if test="${otherOut['badge'] == 1}">selected="selected"</c:if>>蓝字</option>&ndash;%&gt;&ndash;%&gt;--%>
								<%--&lt;%&ndash;&lt;%&ndash;<option value="0" <c:if test="${otherOut['badge'] == 0}">selected="selected"</c:if>>红字</option>&ndash;%&gt;&ndash;%&gt;--%>
							<%--&lt;%&ndash;&lt;%&ndash;</select>&ndash;%&gt;&ndash;%&gt;--%>
						<%--&lt;%&ndash;&lt;%&ndash;</div>&ndash;%&gt;&ndash;%&gt;--%>
						<%--&lt;%&ndash;<div class="cf"></div>&ndash;%&gt;--%>
					  <%--&lt;%&ndash;</li>&ndash;%&gt;--%>
		 				<%--<li class="wordli fl">--%>
							<%--<div class="wordname fl">备注：</div>--%>
							<%--<div class="wordnameinput fl"><input type="text" value="${otherOut['notes']}" id="notes" name="notes" class="toDealInput"> </div>--%>
							<%--<div class="cf"></div> --%>
		 				<%--</li>--%>
		 				<%--<div class="cf"></div> --%>
		 			<%--</ul>--%>
		 	...
		 	<ul class="wordul fr" style="width: 80%;">
 				<li class=" fl">
					...
					<div class="wordnameinput fl">
					<input type="text" value="${otherOut['recordpersonName']}" id="recordPersonName" name="recordPersonName" readonly="readonly">
					<input type="hidden" id="recordPersonUid" name="recordPersonUid" value="${otherOut['recordPersonId']}">
					</div>
					<div class="cf"></div> 
 				</li>
 				<li class=" fl">
					<div class="wordname fl wordfoot">录入日期：</div>
					<div class="wordnameinput fl"><input type="text" value="${fn:substring(otherOut['recordDate'],0,10)}" id="recordDate" name="recordDate" readonly="readonly"> </div>
					<div class="cf"></div> 
 				</li>
 				<%-- <li class="wordli fl">
