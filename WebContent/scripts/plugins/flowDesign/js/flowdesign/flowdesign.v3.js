    ...
    if(processData.list)
    {
        $.each(processData.list, function(i,row) 
        {
           ...
           // var nodeName = 
            ...
            .attr("process_code",row.process_code)
            .attr("process_id",row.id)
            .addClass("process-step btn btn-small")
            .html('<span class="process-flag badge '+badge+'"><i class="'+icon+' icon-white"></i></span>&nbsp;<span id="code_'+nodeId+'">' + (((row.process_code || '')!='')?''+row.process_code+'':'') +'</span>&nbsp;<span id="name_'+nodeId+'">' + row.process_name+'</span>' )
...
//-----外部调用----------------------
    ...
    var Flowdesign = {
        ...
        addProcess:function(row){
                ...
                .attr("style",row.style)
                .attr("process_to",row.process_to) 
                .attr("process_to",row.process_code)
                .attr("process_id",row.id)
                .addClass("process-step btn btn-small")
                .html('<span class="process-flag badge '+badge+'"><i class="'+icon+' icon-white"></i></span>&nbsp;<span id="code_'+nodeId+'">' + (((row.process_code || '')!='')?''+row.process_code+'':'') +'</span>&nbsp;<span id="name_'+nodeId+'">' + row.process_name+'</span>' )
                .mousedown(function(e){
        ...
        updateProcessName:function(activeId,name){
        	$('#name_window'+activeId).html(name);
        },
        updateProcessCode:function(activeId,name){
        	$('#code_window'+activeId).html(name);
        },
        ...
        getProcessJson:function(){
        	try{
                /*连接关系*/ // TODO 有bug（全部删除完会有一个top 0，left 0的节点）
                var aProcessData = {};
                $("#leipi_process_info input[type=hidden]").each(function(i){
                    ...
                    if(processVal.length==2)
                    {
                      ...
                      if(processVal[1] && processVal[1]!='undefined'){
               ...
               var reProcessData = {};
               for(var k in aProcessData){
            	   if(aProcessData[k]['top']!=0 || aProcessData[k]['left']!=0){
            		   reProcessData[k]=aProcessData[k];
            	   }
               }
        ...
        getProcessInfo:function()
        {
            try{
             ...
             return JSON.stringify(reProcessData);
          }catch(e){
              return '';
          }
